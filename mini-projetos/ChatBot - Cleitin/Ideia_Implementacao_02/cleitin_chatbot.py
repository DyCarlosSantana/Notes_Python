...
class MeuApp(ctk.CTk):
    def __init__(self):
        ...
        self.historico_mensagens = [
            {"role": "system", "content": f"Seu nome é Cleitin, um assistente pessoal"}]
        self.title("Cleitin (ChatBot)")
        ...
        self.caixa_chat.insert("0.0", "Olá, eu sou o Cleitin. Como posso te ajudar?\n\n")
        ...
        self.botao_falar = ctk.CTkButton(self.frame_botoes, text=" Falar", width=30, corner_radius=30, fg_color="#005D96", hover_color="#009DE1", command=self.capturar_fala)
    ...
    def capturar_fala(self):
        ...
        with sr.Microphone() as mic:
            ...
            try:
                ...
                self.adicionar_texto_chat("Cleitin:", "Desculpe, não consegui entender o áudio. Tente novamente.")
    ...
    def cleitin(self, mensagem_entrada):
        ...
        if not chave_api:
            self.adicionar_texto_chat("Cleitin", "ERRO: Chave da API do Groq não encontrada!")
            exit()
        ...
        try:
            ...
            resposta_cleitin = resposta.choices[0].message.content
            # Guarda a resposta da IA no histórico com a role 'assistant' para o próximo turno
            self.historico_mensagens.append({"role": "assistant", "content": resposta_cleitin})

        except Exception as e:
            aviso = (f"\n ---- ERRO DESCONHECIDO NA API: ---- \n{str(e)}")
            print(aviso)
        else:
            self.adicionar_texto_chat("Cleitin", resposta_cleitin)
